...
* pairing、auth、session、control、resize，以及旧客户端 WebSocket `encrypted_frame` 兼容路径的 payload。
...
* daemon runtime 只维护 session catalog、workspace/file/git API、pairing/auth、旧 WebSocket `encrypted_frame` 兼容路径，以及到 supervisor 的 watched attachment proxy。
...
* 为旧客户端保留 X25519 + HKDF + ChaCha20Poly1305 的 WebSocket `encrypted_frame` 兼容处理；这不是当前 WebSocket runtime 的安全边界。
* HTTP `/healthz` 和 WebSocket `/ws`。
* 本地 `/local/pairing-token` token 签发入口。
* outbound relay connector：`--relay ws://host:port` 会连接可信 relay，并通过它传输明文 WebSocket/HTTP tunnel 应用数据。
...
`termrelay` 是可信 admission/routing 层，当前启动入口是：
...
relay 在统一 `/ws` 上解析连接前奏 `route_hello`，校验 admission，并用其中的 `server_id`、`role`、`route_generation` 和 data pipe token 完成路由绑定。旧 path-based relay 路由不再作为当前入口；建链后 relay 转发明文 WebSocket/HTTP tunnel 应用数据，不持有 session/PTY 状态，也不参与 daemon 负责的 pairing/auth/session/control 最终权限判断。
...
* workspace package 版本与 Git tag 保持一致，release 资产使用同一个 tag。
...
* GitHub Actions 在 tag push 时只发布 `termd`、`termctl`、`termrelay` 的 Linux amd64/arm64 六个裸二进制，不构建 tarball 或容器镜像。
...
direct 模式中，当前 Web WebSocket 直接发送明文 JSON/binary protocol packet，不执行 X25519 runtime key exchange，也不把 WebSocket RPC 放入 `encrypted_frame`。terminal attach bootstrap 仍由 daemon 完成权限校验和 stream 建立；bootstrap 成功后，终端输入/输出统一编码为 attach frame，由 supervisor 直接定义、发送、接收和关闭。这里的 frame 封装不表示 WebSocket runtime E2EE；daemon 不再重放 daemon-side terminal mirror，也不再裁决 terminal heartbeat。
...
relay runtime E2E 覆盖真实 `termrelay` 进程、真实 `termd --relay` 进程，以及 Web 经 relay client URL 访问 daemon。当前 Web 应用流量以明文经过可信 relay；relay 负责 admission/routing，但 pairing/auth/session 权限仍由 daemon 最终校验。
...
| WebSocket E2EE `encrypted_frame` | 旧客户端兼容路径，已实现并测试；当前 Web WebSocket runtime 不使用 |
| Noise protocol | 未实现；当前 Web WebSocket runtime 不执行 Noise 或 X25519 handshake |
...
| `termrelay` trusted admission/routing | 已实现并通过 E2E 验证；转发明文应用流量，不持有 session/PTY 状态 |
