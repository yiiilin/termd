...
* 终端渲染只使用 Ghostty Web renderer；如果 Ghostty WASM 资源加载失败，前端直接暴露错误，不再回退到第二套终端实现。`TerminalPane` 不直接依赖 renderer 私有 DOM。
...
| `termctl pair/new/attach/control/resize/list` | 旧 CLI 能力；当前交互 attach 降级为调试 |
...
* UI 通过 Ghostty renderer 渲染终端，不能依赖 renderer 私有 DOM 作为业务语义。
