...
协议类型不包含 UI 逻辑，也不表达 RBAC、多用户或企业权限。
...
* outbound relay connector：`--relay ws://host:port` 会连接 relay 的 `/daemon-mux`。
...
| TLS、relay auth、自动重连、多 relay、公网部署配置 | 未实现 |
| RBAC、多用户、审计、文件传输、session 录像、SSO、Kubernetes | 非本轮 MVP |
...
* UI 不实现 daemon 业务逻辑，不判断 controller/viewer 权限。
