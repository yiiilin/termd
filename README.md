...
- 主机需要 `sudo`、`curl`、`sha256sum`、`python3`、systemd 和常见账户管理工具。
...
```bash
curl -fL https://github.com/yiiilin/termd/releases/latest/download/termd-linux-amd64 \
  -o termd-linux-amd64
curl -fL https://github.com/yiiilin/termd/releases/latest/download/checksums.txt \
  -o checksums.txt
sha256sum --ignore-missing --check checksums.txt
chmod 0755 termd-linux-amd64
./termd-linux-amd64 install --dry-run --web --user "$(id -un)"
sudo ./termd-linux-amd64 install --web --user "$(id -un)"
rm -f termd-linux-amd64 checksums.txt
```

`--dry-run` 只显示非敏感安装计划。第二条 `install` 命令会再次显示计划并交互确认。需要 HTTP/SOCKS 代理、安装 relay/CLI 或使用 arm64 时，按[完整安装指南](docs/installation.md)执行。
