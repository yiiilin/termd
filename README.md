...
- relay 网络：`termrelay` 是 dumb pipe，按 `server_id` 转发 WebSocket；`termd` 可主动连接一个或多个 relay。
...
`termrelay` 是不可信转发层，只按 `server_id` 转发 WebSocket frame，不解密、不解析业务内容。它也提供 [docker-compose 部署方式](docs/deployment.md#termrelay-docker-compose)。
