...
interface CreateTerminalOutputDrainOptions {
  ...
  isTerminalPinnedToBottom: (terminal: Terminal) => boolean;
  scrollToBottom: () => void;
  scheduleScrollToBottom: (passes?: number) => void;
  queueCursorReport: (options?: { immediate?: boolean }) => void;
  scheduleMobileScrollPosition: (options?: { immediate?: boolean }) => void;
...
export function useTerminalOutputWriter(
  ...
  const createTerminalOutputDrain = useCallback((options: CreateTerminalOutputDrainOptions) => {
    ...
    const afterTerminalMutation = () => {
      ...
      options.scheduleMobileScrollPosition();
      ...
      if (outputQueueIdle) {
        options.requestTrackedFrame(() => {
          if (shouldScrollBottomAfterMutation) {
            ...
            options.scheduleScrollToBottom(4);
          }
      ...
      options.requestTrackedFrame(() => options.terminal.refresh(0, Math.max(0, options.terminal.rows - 1)));
    };
