...
test("relay Web 双客户端同会话不同分辨率轮番离线上线后仍能恢复", async ({ page, browser }, testInfo) => {
  ...
  try {
    ...
    await runTerminalCommand(page, `printf '${marker(name)}-shared-after-reconnect-client-a\\n'`);
    ...
    await expectTerminalScrollAtBottom(page);
    await expectTerminalScrollAtBottom(secondPage);
    await runTerminalCommand(secondPage, `rm -f '${bulkDoneFile}'; printf '${marker(name)}-shared-after-reconnect-client-b\\n'`);
