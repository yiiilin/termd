...
prepare_release_fixture_setup() {
  ...
  mkdir -p "${PREPARE_REPO}/scripts" "${PREPARE_REPO}/termui/frontend" \
    "${PREPARE_REPO}/docs/releases" "$PREPARE_BIN"
...
if [[ "${1:-}" == "generate-lockfile" ]]; then
  ...
  if [[ "${PREPARE_RELEASE_FIXTURE_CARGO_MODE:-}" == "fail-generate-lockfile" ]]; then
    printf 'fixture cargo generate-lockfile failure\n' >&2
    exit 1
  fi
  exit 0
fi
...
test_prepare_release_generation_failure_is_isolated() (
  prepare_release_fixture_setup
  trap prepare_release_fixture_teardown EXIT
  local before after base
  prepare_release_paths 9.9.4
  write_prepare_release_fixture_notes 9.9.4 "lockfile failure 隔离回归。"
  printf 'staged unrelated edit\n' >>"${PREPARE_REPO}/README.md"
  git -C "$PREPARE_REPO" add -- README.md
  before="$(snapshot_prepare_release_caller "$PREPARE_REPO" "${PREPARE_RELEASE_PATHS[@]}" README.md)"
  base="$(git -C "$PREPARE_REPO" rev-parse HEAD)"

  export PREPARE_RELEASE_FIXTURE_CARGO_MODE=fail-generate-lockfile
  run_prepare_release_fixture 9.9.4 --allow-dirty --skip-verify
  unset PREPARE_RELEASE_FIXTURE_CARGO_MODE
  [[ "$PREPARE_STATUS" -ne 0 ]]
  [[ "$PREPARE_OUTPUT" == *"fixture cargo generate-lockfile failure"* ]]
  after="$(snapshot_prepare_release_caller "$PREPARE_REPO" "${PREPARE_RELEASE_PATHS[@]}" README.md)"
  assert_prepare_release_snapshot_unchanged "generation failure" "$before" "$after"
...
run_test test_prepare_release_generation_failure_is_isolated
