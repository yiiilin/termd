...
## 六、2026-06-04 Ghostty/tmux 重构执行状态

本节记录本轮按新目标推进的可执行分片：Ghostty 作为 Web 终端渲染方向，tmux 作为终端会话守护程序，termd 作为会话分发，relay 继续保持 dumb pipe，Web 作为正式客户端。
...
- [x] Slice 1: 架构与状态版本门禁
  - 结果：
    - 文档改为 tmux-backed 目标边界，避免把尚未接入的 tmux 生产路径写成当前实现。
    - `STATE_SCHEMA_VERSION` 升到 2，旧 JSON / 旧 SQLite / client-history-only 旧库有内容时不再静默迁移。
    - `StateStore`、`ClientHistoryStore`、HTTP upload 状态入口增加 v2 兼容门禁。
    - 生产启动顺序改为先 `StateStore::load`，再 upload cleanup；不再 adopt live supervisor，只保留 orphan warning，并继续 prune closed rows。
    - 旧 startup adopt integration test 改为 no-adopt/orphan 语义。
  - 验证：`cargo fmt --all`
  - 验证：`cargo test -p termd client_history -- --nocapture`
  - 验证：`cargo test -p termd state_ -- --nocapture`
  - 验证：`cargo test -p termd --test session_supervisor daemon_startup_warns_but_does_not_adopt_unrecorded_live_supervisor -- --nocapture`
  - 验证：`cargo check -p termd`
  - 验证：`cargo test -p termd`
  - 复审：Darwin PASS
  - 复审：Bohr PASS
- [x] Slice 2: tmux host 后端
  - 目标：实现 `PtyBackend`/`PtySession` 兼容的 tmux backend；tmux session 是持久真相源，当前切片先用 daemon runtime 的 `tmux attach-session` PTY bridge 复用现有协议层 fanout。
  - 边界：每个 Web attach 独立创建短生命周期 tmux client 尚未实现，保留到后续 Web attach runtime 切片，不把它计入 Slice 2 完成条件。
  - 结果：
    - 新增 `TmuxPtyBackend`，用 state 边界内的 tmux socket 创建 `new-session`，并用 daemon runtime attach PTY bridge 转发 I/O。
    - 生产默认 `DefaultDaemonProtocol` 切到 tmux backend；旧 supervisor 生产启动路径只保留 orphan warning，不再 adopt。
    - `PtyRestoreInfo::Tmux` 支持 SQLite 持久化、startup reconnect gate、保存尺寸恢复和真实 tmux runtime reconnect。
    - daemon attach bridge drop 时先 detach tmux client，避免丢弃 runtime 句柄时带掉 tmux host session。
    - server 测试 state 目录隔离，避免并发测试共享 tmux socket。
  - 验证：`cargo fmt --all`
  - 验证：`cargo check -p termd -p termrelay`
  - 验证：`cargo test -p termd --test tmux_backend -- --nocapture`
  - 验证：`cargo test -p termd startup_restores_tmux_restore_info_records -- --nocapture`
  - 验证：`cargo test -p termd`
  - 验证：`git diff --check`
  - 复审：Descartes PASS
  - 复审：Boole PASS
- [x] Slice 3: recovery 与 snapshot
  - 目标：用 tmux session/pane 状态替代 supervisor restore_info；使用 `tmux capture-pane` / pane metadata 做 attach snapshot 和恢复判定。
  - 结果：
    - startup reconnect gate 接受 `PtyRestoreInfo::Tmux`，并在恢复失败时关闭 stale runtime/history/state。
    - `SessionRuntime::reconnect_session` 把持久化尺寸传给 backend；tmux backend reconnect 后同步 `resize-window`。
    - `snapshot()` / `terminal_snapshot()` 使用 `tmux capture-pane -p -e -J -S -1000`，并通过 `#{pane_pid}` / `#{pane_current_path}` 暴露 pane metadata。
    - tmux bridge drop 改为按 attach client pid 定位 `client_name` 后只 detach 自己的 client，不再 session 级踢掉其他客户端。
    - `next_terminal_seq` 保证 snapshot `base_seq` 与后续 live output 单调衔接，避免 frame log 丢弃 snapshot 之后的输出。
  - 验证：`cargo fmt --all`
  - 验证：`cargo test -p termd --test tmux_backend -- --nocapture`
  - 验证：`cargo check -p termd -p termrelay`
  - 验证：`cargo test -p termd`
  ...
  - 复审：Halley PASS
- [x] Slice 4: 文件面板与 cwd
  - 目标：文件面板 cwd 从 tmux pane metadata 获取，保留安全路径边界和已 attach 才能操作 session 的不变量。
  - 结果：
    - tmux backend 通过 `#{pane_current_path}` 暴露 pane cwd，daemon runtime 将 cwd 透传给 protocol。
    - `session_files` 无显式 path 时先同步 terminal cwd，再回退到 client history / file panel 持久位置。
    - terminal cwd 不可读或不可用时会清除 session cwd cache，避免复用 stale cwd 覆盖用户手动浏览位置。
    - `list_session_files` 入口继续保留 `require_attached_session`，未 attach 连接不能操作 session 文件面板。
  - 验证：`cargo fmt --all`
  - 验证：`cargo test -p termd --test tmux_backend -- --nocapture`
  - 验证：`cargo test -p termd session_files_without_path_ -- --nocapture`
  - 验证：`cargo check -p termd -p termrelay`
  - 验证：`git diff --check`
  - 复审：Volta PASS
  - 复审：Chandrasekhar PASS
- [x] Slice 5: Web renderer adapter + ghostty-web
  - 目标：Web terminal renderer 通过 adapter 接入 Ghostty/xterm 兼容实现，UI 不依赖 renderer 私有 DOM。
  - 结果：
    - 新增 `TerminalRendererAdapter` 边界，默认继续使用 xterm；`VITE_TERMD_TERMINAL_RENDERER=ghostty` 时动态加载 `ghostty-web`。
    - `TerminalPane` 不再直接 import `@xterm/*`、查询 xterm 私有 DOM 或调用 xterm private core；xterm textarea 锚定逻辑收口到 xterm adapter。
    - ghostty adapter 补齐 `onWriteParsed`、`refresh`、`setOptions`、`scrollState`、`scrollToLine` 与 `write` 兼容层，避免 ghostty 模式下运行期崩溃、options 整体替换和上滚后写入强制贴底。
    - xterm search/fit/focus/write/reset/resize 默认路径保持不变；ghostty 暂无 SearchAddon 时 search adapter 为 no-op。
    - 新增 ghostty adapter 单测，覆盖 init 缓存、contract shim、options mutation、scroll 坐标归一化和写入时 scrollback 位置恢复。
  - 验证：`npm run typecheck`
  - 验证：`npm test -- --run src/__tests__/terminal-pane.test.tsx`
  - 验证：`npm test -- --run src/__tests__/ghostty-renderer.test.ts`
  - 验证：`npm test -- --run src/__tests__/app.test.tsx -t '可以创建 session 并自动 attach 到 terminal|attach WebSocket 短断恢复后还能继续渲染新的 terminal frame|新 attach 的输出必须等 TerminalPane reset 确认后才写入 xterm|持续输出时合并写入 xterm'`
  - 验证：`npm run build`
  - 验证：`git diff --check`
  - 复审：Kierkegaard PASS
  - 复审：Franklin PASS
- [x] Slice 6: relay/Web E2E 收口
  - 目标：direct/relay Web 路径都覆盖 tmux-backed create/list/attach/input/resize/reconnect；relay 只转发密文和 opaque frame。
  - 结果：
    - 前端新增 `exerciseTmuxBackedWebLifecycle` 验收 helper，direct 与 relay `/ws` 路径共用同一套 create/list/attach/input/resize/reconnect 断言。
    - direct/relay Web 用例都覆盖 `terminal.create` 自动 attach、不追加重复 `terminal.attach`、文件面板 no-path cwd、terminal input E2EE outer-wire 不泄漏、resize 发送、断线后 reattach 和后续 terminal frame 渲染。
    - relay Rust E2E 继续覆盖 raw data pipe、idle daemon data pipe、业务形状 text/binary opaque 转发、真实 encrypted_frame 不改写、大 snapshot binary 原样转发和 server_id 隔离。
    - relay 生产边界保持 dumb pipe：只在 route prelude 阶段解析路由元数据，业务 payload 仍由 daemon/client E2EE 处理。
  - 验证：`npm test -- --run src/__tests__/app.test.tsx -t 'direct Web 路径串联 tmux-backed session|relay /ws 路径串联 tmux-backed session'`
  - 验证：`cargo test -p termrelay --test relay_e2e -- --nocapture`
  - 验证：`cargo check -p termd -p termrelay`
  - 验证：`npm run typecheck`
  - 验证：`git diff --check`
  - 复审：Sartre PASS
  - 复审：Gauss PASS
- [x] 最终收口：全量验证、最终双审和文档同步
  - 结果：
    - README/TECH 当时已同步 Slice 6 真实边界：tmux-backed 默认 backend、Web-first client、可选 `ghostty-web` renderer、统一 `/ws + route_hello` relay 入口、relay dumb pipe；后续 Slice 7 继续补齐 watched Web attach 的 tmux client lifecycle。
    - 最终复审指出的 async renderer 迟到清 host 竞态已修复：迟到 renderer 只 dispose 自己，不再清空 host，并新增 race 回归测试。
    - 最终复审指出的 TECH 旧 relay path-based 文档残留已修复：当前文档只描述统一 `/ws` 和 `route_hello` 路由前奏。
  - 验证：`cargo fmt --all`
  - 验证：`cargo test -p termd`
  - 验证：`cargo test -p termrelay`
  - 验证：`npm test -- --run`
  - 验证：`npm run typecheck`
  - 验证：`npm run build`
  - 验证：`git diff --check`
  - 最终复审：Lorentz PASS
  - 最终复审：Euler PASS
- [x] Slice 7: watched Web attach 独立 tmux client lifecycle
  - 目标：
    - `watch_updates=true` 的 Web attach 在 runtime 中创建独立、可释放的 tmux client handle。
    - permission-only attach 只获得 session RPC 权限，不创建终端输出 watcher client。
    - WebSocket/terminal stream 取消或连接断开时只释放本连接自己的 tmux client，不 close session host，不撤销同设备其他在线连接的 operator 角色。
  - 边界：
    - 本切片不重写现有 session 级 terminal frame log / snapshot / tail 输出管线，避免把 per-client tmux 输出流与 session 级 `terminal_seq` 混用。
    - tmux client handle 仅表达真实 tmux attach 生命周期；terminal 明文仍经 daemon E2EE 输出路径分发，relay 仍不可解密。
  - 结果：
    - 新增 `PtyAttachment` / `PtyBackend::attach_client` 生命周期接口，runtime 以 attachment id 保存连接级 watched handle。
    - tmux backend 用 `tmux -C attach-session` 创建 watched Web attach 的 control-mode client，drop/detach 只释放自己的 client pid，不杀 tmux session host。
    - `watch_updates=true` 的 Web terminal create/attach 会登记 watched attachment；`watch_updates=false` 只获得 session RPC 权限，不创建 watcher client。
    - terminal stream 替换、取消、结束和 WebSocket 断开只释放当前连接自己的 watched attachment；session close/discard 才清理全部 watched handles。
    - `create_session_inner` 在 runtime create 后遇到后续失败会 rollback hidden runtime/tmux host；tmux attachment 构造失败会关闭 stdin、停止 child 并 join drain thread。
    - README/TECH 已同步当前边界：watched attach 有独立 tmux client lifecycle handle，但 terminal 输出仍沿用 daemon bridge/session-level fanout 与 `terminal_seq`。
  - 验证：
    - tmux integration：一个 session host 加两个 watched attach 后，tmux client 数随 attach/drop 精确变化，drop attach 不杀 host。
    - runtime unit：watched attach 创建 attachment handle，permission-only attach 不创建；detach/close/discard 清理 handle。
    - protocol unit：`session_attach.watch_updates=true` 注册 attachment，`watch_updates=false` 不注册；terminal stream 替换、取消和连接断开会释放对应 attachment。
    - `cargo fmt --all`
    - `cargo test -p termd`
    - `cargo test -p termrelay`
    - `cargo check -p termd -p termrelay`
    - `npm test -- --run`
    - `npm run typecheck`
    - `npm run build`
    - `git diff --check`
